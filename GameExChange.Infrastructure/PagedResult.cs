...
namespace GameExChange.Infrastructure
{
    public class PagedResult<T> : IEnumerable<T>, ICollection<T>
    {
        ...
        public PagedResult() => this.PageData = new List<T>();
        ...
        public override int GetHashCode() => this.TotalPages.GetHashCode() ^
        ...
        public static bool operator != (PagedResult<T> a, PagedResult<T> b) => !(a == b);
        #endregion

        #region IEnumberable Members
        public IEnumerator<T> GetEnumerator() => PageData.GetEnumerator();

        IEnumerator IEnumerable.GetEnumerator() => PageData.GetEnumerator();
        #endregion

        #region ICollection Members
        public void Add(T item) => PageData.Add(item);

        public void Clear() => PageData.Clear();

        public bool Contains(T item) => PageData.Contains(item);

        public void CopyTo(T[] array, int arrayIndex) => PageData.CopyTo(array, arrayIndex);

        public int Count => PageData.Count;

        public bool IsReadOnly => false;

        public bool Remove(T item) => PageData.Remove(item);
        #endregion 
    }
}
